    ...
    with torch.no_grad():
        for _, (inputs, targets) in enumerate(tqdm(loader)):
            ...
            all_features=pd.concat([all_features, pd.DataFrame(features.data.cpu().numpy())],
                                         ignore_index=True) #每张图片的特征向量
            all_logits = pd.concat([all_logits, pd.DataFrame(logits.data.cpu().numpy())],
                                         ignore_index=True) #每张图片的原始logits（未归一化的模型输出，通常是10维，代表每个分数区间的置信度）
            all_outputs = pd.concat([all_outputs, pd.DataFrame(outputs.data.cpu().numpy())],
                                         ignore_index=True) #每张图片的输出分数（通常是10维，代表每个分数区间的预测分数）
            if opt['loss_type'] == 'emd':
                preds.extend(get_score(outputs)) #预测分数
                labels.extend(get_score(targets)) #真实分数
...
def prepare_dataloader(opt):
    ...
    val_dataset = AVADataset(os.path.join(opt['csv_path'], 'test.csv'), opt['dataset_path'], opt['st'], mode='val',
                             loss_type=opt['loss_type'])
