...
def simloss(features, targets):
    print(f"simloss called: targets.shape={targets.shape}, dtype={targets.dtype}")

    x = features

    # 自动处理 targets 是分数还是分布
    if targets.dim() == 2 and targets.size(1) == 10:
        # 10维打分分布
        y = torch.Tensor(get_score(targets)).unsqueeze(1)
    elif targets.dim() == 1:
        # 已经是分数（均值），直接使用
        # y = targets.unsqueeze(1)
        y=targets*torch.ones((features.shape[0],features.shape[0]),device='cuda:0')
    else:
        raise ValueError(f"simloss expects targets as shape [B] or [B,10], but got {targets.shape}")
